...
#endif
	    memcpy(&explicit_cell_server_list.xhostAddr[i],
		   &explicit_cell_server_list.hostSA[i].u.in,
		   sizeof(explicit_cell_server_list.xhostAddr[i]));
	    explicit = 1;
	} else
	    break;
    UNLOCK_GLOBAL_MUTEX;
		    ...
		    struct ubik_client ** conn)
{
    ...
    serverconns[0] =
	rx_GetCachedConnection2(&cellinfo.hostSA[match], service, sc, si);
#else
    serverconns[0] =
	rx_NewConnection2(&cellinfo.hostSA[match], service, sc, si);
#endif
...
#ifdef AFS_PTHREAD_ENV
	serverconns[i] =
	    rx_GetCachedConnection2(&cellinfo->hostSA[i], service, sc, si);
#else
	serverconns[i] =
	    rx_NewConnection2(&cellinfo->hostSA[i], service, sc, si);
#endif
