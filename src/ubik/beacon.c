			     ...
			     char clones[], afs_uint32 aservers[])
{
	...
	for (i = 0; i < info->numServers; i++) {
	    if (ntohl((afs_uint32) info->hostSA[i].u.in.sin_addr.s_addr) ==
	...
	for (i = 0; i < info->numServers; i++) {
	    ...
	    ubik_servers = ts;
	    ts->addr[0] = info->hostSA[i].u.in.sin_addr.s_addr;
	    if (clones[i]) {
		ts->isClone = 1;
	    } else {
		if (!magicHost
	    ...
	    /* for vote reqs */
/* NOTE TO MIKE: wasn't sure if this could be converted to NewConnection2 due to ubik_callPortal */
	    ts->vote_rxcid =
		rx_NewConnection(info->hostSA[i].u.in.sin_addr.s_addr,
				 ubik_callPortal, VOTE_SERVICE_ID,
				 addr_globals.ubikSecClass, addr_globals.ubikSecIndex);
	    /* for disk reqs */
	    ts->disk_rxcid =
...
verifyInterfaceAddress(afs_uint32 *ame, struct afsconf_cell *info,
	...
	for (i = 0; i < totalServers; i++) {
	    if (info)
		tmpAddr = (afs_uint32) info->hostSA[i].u.in.sin_addr.s_addr;
	    else
		tmpAddr = aservers[i];
	    if (myAddr[j] == tmpAddr) {
		*ame = tmpAddr;
